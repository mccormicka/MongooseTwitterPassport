...
//                    //In a future release
    ...
    function addSchemaExtensions() {
        ...
        /**
         * Archive a Twitter Account.
         * It can be unarchived by re authenticating.
         * @param id
         * @param next
         */
        schema.methods.archiveTwitterAccount = function (id, next) {
            if (!_.isFunction(next)) {
                next = noop;
            }
            getExtension(this.db).findOne({modelId: this._id, 'profile.id': id}, function (err, result) {
                if (err) {
                    log.error('Error finding Twitter account ', id);
                    next(err, result);
                }
                if (result) {
                    result.update({active: false}, function (err, result) {
                        log.debug('deactivated Twitter Account', err, result);
                        next(err, result);
                    });
                } else {
                    next(err, result);
